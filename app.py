...
if api_key:
    st.title("🔥 蝦皮獨家爆款 AI 戰術系統 (差異化客製版)")
    ...
    with col1:
        ...
        competitor_flaw = st.text_input("競品常見痛點/缺點 (差異化關鍵)", "市售常見精華液太黏膩、吸收慢、容易起屑")
        selling_points = st.text_area("本品獨家賣點", "極速吸收、專利控油成分、敏弱肌專用")
        uploaded_file = st.file_uploader("上傳商品圖片 (選填)", type=["jpg", "jpeg", "png"])
        
        btn = st.button("🚀 生成獨家降維打擊行銷包", type="primary", use_container_width=True)
...
（直接對標競品痛點，突出獨家解決方案，含熱搜字與 Emoji）

---
### 🎯 2. 五感沉浸式商品描述 (反罐頭文案)
（包含：【競品避坑指南】、【痛點解決】、【實測體驗】、【售後無憂條款】）

---
### 📈 3. 蝦皮搜尋演算法關鍵字矩陣
（包含高流量詞、精準長尾詞、轉化詞標籤，以 # 分隔）

---
### 🎬 4. TikTok / Shopee 衝動購物型 15秒黃金前3秒短影音分鏡
（設計前 3 秒強烈鉤子 Hook、畫面動作、台詞與背景音樂）

---
### 🎨 5. 差異化產品攝影與社群梗圖英文 Prompt (Midjourney / Jimeng)
（1組高端商業攝影 Prompt + 1組高互動視覺梗圖 Prompt）
"""
                    parts = [{"text": prompt}]
